...
state: artifacts/prior_state_v7.joblib
모델: artifacts/models/{lgbm,xgb,svm,rf}_v9_model.joblib
"""
...
from config import ARTIFACTS_DIR, ENSEMBLE_WEIGHTS_V9, MODELS_DIR
...
_STATE_PATH = ARTIFACTS_DIR / "prior_state_v9.joblib"
_FEATURE_NAMES_PATH = MODELS_DIR / "feature_names_v9.json"
...
def _load():
    global _state, _lgbm, _xgb, _svm, _rf, _feature_names
    if _state is None:
        _state = joblib.load(_STATE_PATH)
        _lgbm = joblib.load(MODELS_DIR / "lgbm_v9_model.joblib")
        _xgb  = joblib.load(MODELS_DIR / "xgb_v9_model.joblib")
        _svm  = joblib.load(MODELS_DIR / "svm_v9_model.joblib")
        _rf   = joblib.load(MODELS_DIR / "rf_v9_model.joblib")
...
def predict_one_v9(input_dict: dict) -> dict:
    """
    input_dict = {
        "map": str,
    ...
    p_svm   = float(_svm.predict_proba(X)[0, 1])
    p_rf    = float(_rf.predict_proba(X)[0, 1])
    p_ens   = (w["lgbm"] * p_lgbm + w["xgb"] * p_xgb
               + w["svm"] * p_svm + w["rf"] * p_rf)
