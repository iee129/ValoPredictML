"""v10 모델 평가: VCT+VCL 통합 학습 모델 테스트.

  python -m src.evaluate_v10

산출:
  artifacts/reports/eval_summary_v10.json
"""
...
from config import ENSEMBLE_WEIGHTS_V10, MODELS_DIR, PROCESSED_DIR, REPORTS_DIR
...
def main():
    ...
    print("[eval_v10] loading test_v10.csv", file=sys.stderr)
    X_test, y_test = _load_xy(TEST_CSV_V10)
    print(f"[eval_v10] X_test {X_test.shape}  A승률(test)={np.mean(y_test):.3f}",
          file=sys.stderr)

    print("[eval_v10] loading v10 models ...", file=sys.stderr)
    lgbm = joblib.load(MODELS_DIR / "lgbm_v10_model.joblib")
    xgb  = joblib.load(MODELS_DIR / "xgb_v10_model.joblib")
    rf   = joblib.load(MODELS_DIR / "rf_v10_model.joblib")

    proba_lgbm = lgbm.predict_proba(X_test)[:, 1]
    proba_xgb  = xgb.predict_proba(X_test)[:, 1]
    proba_rf   = rf.predict_proba(X_test)[:, 1]
    w = ENSEMBLE_WEIGHTS_V10
    proba_ens = w["lgbm"] * proba_lgbm + w["xgb"] * proba_xgb + w["rf"] * proba_rf

    rows = [
        _metrics("lgbm", y_test, proba_lgbm),
        _metrics("xgb",  y_test, proba_xgb),
        _metrics("rf",   y_test, proba_rf),
        _metrics(f"ensemble({w['lgbm']:.2f}/{w['xgb']:.2f}/{w['rf']:.2f})",
                 y_test, proba_ens),
    ]
    ...
    print(f"\n[eval_v10] best = {best['name']} "
          f"(Acc={best['accuracy']:.4f}, ROC-AUC={best['roc_auc']:.4f})", file=sys.stderr)
    print(f"[eval_v10] majority 대비 +{delta:.4f}", file=sys.stderr)
    ...
    summary = {
        ...
        "baselines": {
            "majority_A_always":  {k: baseline_majority[k] for k in baseline_majority if k != "name"},
            "random_coin_seed42": {k: baseline_random[k]   for k in baseline_random   if k != "name"},
        },
        "best_model_vs_majority_accuracy_delta": delta,
    }
    out_path = REPORTS_DIR / "eval_summary_v10.json"
    with open(out_path, "w") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)
    print(f"[eval_v10] saved {out_path}", file=sys.stderr)
