...
> `agents_a`/`agents_b`는 `ml/advanced/preprocess.py`의 `_parse_agents`와 동일하게 파싱(`normalize_agent` + `_agent_col_key`). 역할은 `AGENT_ROLE_MAP`.

---

## 2. 빌더 — `ml/insights/build_insights.py` (신규)

> `ml/insights/`에 두면 `app/`·`ml/`처럼 커밋 가능 영역이다(`.gitignore` 허용목록). 산출 JSON은 `reports/insights/`(로컬 전용)로 나간다.
...
| `valo_web_backend/data/agent_map_rules.json` (손수 인코딩, 커밋) | `/agent-map-fit` 룰 fallback 병합 |

`valo_web_backend/data/agent_map_rules.json`은 `docs/10_valorant/{agents,maps}.md`의 "강한 맵 / 키 픽 / 약한 요원"을 사람이 1회 구조화한다(데이터로 안 잡히는 신규 요원 대비).
...
```python
# valo_web_backend/services/insights.py
import json
...
- 데모 런북에 한 줄 추가: 모델 학습 후 `python -m ml.insights.build_insights` 실행 → [../04_integration/02_demo_runbook.md](../04_integration/02_demo_runbook.md).
