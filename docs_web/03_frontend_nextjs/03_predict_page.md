...
핵심 시연 페이지. **맵 + 기준연도 + 팀당 5×{선수, 요원}** 을 입력받아 `POST /predict` 호출. 09_web 예측 페이지와의 결정적 차이는 **각 슬롯에 선수 입력이 있다**는 점.
...
> 규칙 출처: `app/predict.py` `_validate_slots`. 프론트 검증은 편의일 뿐, **최종 권위는 백엔드**다(우회 입력은 백엔드가 422로 차단).
...
> 첫 호출은 콜드스타트로 수 초 걸릴 수 있다 → 로딩 스피너에 "이전 연도 기록을 계산 중입니다(첫 실행은 다소 걸립니다)" 문구 권장(Streamlit 앱과 동일 안내).
