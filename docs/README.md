...
| [04_data_processing](04_data_processing/) | 전처리 파이프라인 — 파서 → 품질 검사 → 피처 → 분할 |
...
- **Baseline ML 파이프라인**: 완료 — Kaggle-only previous-year 178피처, train(80%) 학습, Test AUC=0.6587
...
- **심화 모델 (adv_kaggle_only)**: 완료 — RF+XGB+LGBM Optuna HPO, Ensemble Test AUC=0.7570, Acc=0.6958, F1=0.7649, verdict PASS_TRUSTED_KAGGLE_ONLY_ADVANCED
- **Streamlit UI**: 완료 (`app/main.py` + `app/predict.py`)
- **기말 발표**: 2026-06-09
