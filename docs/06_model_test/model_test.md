> ⚠️ 참고/확장 설계: 현재 시연은 웹 스택(FastAPI `src/api` + Next.js `web`) 기준이다. 이 문서의 FastAPI 테스트 설계는 참고용으로 보존한다.
...
## 3. FastAPI 구현 코드 (미구현 예시 — 실제 서빙은 `src/inference/predict.py`가 `models/advanced/ensemble.joblib` 단일 로드)
...
### 3.3 prediction_service.py (미구현 참고용 — 실제 서빙: `src/inference/predict.py`, `models/advanced/ensemble.joblib` 단일 로드)
...
| 문서 | 경로 | 내용 |
|------|------|------|
| ML 개념 검증 | [`ml_concept_validation.md`](./ml_concept_validation.md) | GroupKFold·앙상블·중요도·알고리즘 선택 |
| 프로젝트 차별점 | [`project_differentiation.md`](./project_differentiation.md) | 5개 차별점 + 2모델 성능표 |
| 검증 결과 종합 | [`verification_summary.md`](./verification_summary.md) | 2모델 성과지표 종합 |
| 2모델 평가 (개요) | [`07_model_evaluation/00_overview.md`](./07_model_evaluation/00_overview.md) | 2모델 정의 · 분할/모델 축 차이 |
| 베이스라인 (랜덤 holdout) | [`07_model_evaluation/01_random_baseline.md`](./07_model_evaluation/01_random_baseline.md) | 421피처 · Test AUC 0.5943 |
| 심화 (시간순 holdout) | [`07_model_evaluation/04_chrono_advanced.md`](./07_model_evaluation/04_chrono_advanced.md) | 179피처 · Test AUC 0.7010 |
| 2모델 비교 | [`07_model_evaluation/05_cross_model_comparison.md`](./07_model_evaluation/05_cross_model_comparison.md) | 분할·모델 축 비교 |
