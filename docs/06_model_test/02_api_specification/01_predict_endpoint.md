...
POST /predict 호출 시 예측 결과는 PostgreSQL `prediction_history` 테이블에 자동 저장됩니다.

- `VALO_DATABASE_URL` 또는 `DATABASE_URL` 미설정 시 경고 로그 후 skip — 예측 응답에는 영향 없음
- DB 장애 발생 시에도 동일하게 graceful skip (503 반환 없음)
- 컬럼: `request_json`(JSONB), `response_json`(JSONB) 포함

```sql
-- prediction_history 테이블 구조 (services/history.py가 자동 생성)
