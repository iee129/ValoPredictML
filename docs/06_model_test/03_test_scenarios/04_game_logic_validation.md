...
```bash
# 미구현 — FastAPI 범위 외. 동등 검증: uvicorn api.main:app 후 cd web && npm run dev → 홈(커스텀 5v5)에서 직접 입력
...
```bash
# 미구현 — FastAPI 범위 외. 웹 홈(커스텀 5v5)에서 직접 검증 가능
...
```bash
# 미구현 — FastAPI 범위 외. 웹 홈(커스텀 5v5)에서 맵 변경하며 직접 검증 가능
