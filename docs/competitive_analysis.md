...
- 8개 중 **prematch + 조합 단위 + 누수 방지** 셋을 모두 만족하는 프로젝트는 없음.
...
1. **prematch 시점에 모델 기반 승률 예측을 제공하는 도구가 사실상 없다**
   ...
   - GitHub ML 프로젝트 8개 중 prematch + 조합 단위 + 누수 방지를 모두 만족하는 것 없음.
...
- **henrikdev API** — VLR.gg 데이터 비공식 API 래퍼, 다수 오픈소스 도구의 백엔드로 쓰임 (`docs/07_data/04_api_sources/01_henrikdev_api.md` 참조)
