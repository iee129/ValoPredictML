...
```
[데이터 소스: Kaggle CSV 5개 — 외부 API 미사용]
  ...
  piyush86kumar/valorant-champions-2024 (소스 가중치 1.5)
         |
         v
  [Phase 1: 파싱 — ml/raw_preprocess.py parser family]
  ...
  [Phase 6: 피처 사전 집계 — train.csv만 사용]
    ...
    → val/test에 join (신규 조합: winrate=0.5, experience=0)
         ...
         v
  [Phase 7: 피처 엔지니어링]
    baseline 계약: 178개 피처 (맵 원핫 13 + 역할군 12 + 요원 84 + 선수 prior 24 + 시너지 3 + 맵×요원 21 + 선수×요원 21)
    advanced 계약: 128개 피처 (diff 제거, miks 29번째 요원 추가)
...
| 단계 | 스크립트 | 목적 | 주요 출력 |
|------|----------|------|-----------|
| 파싱 | `ml/raw_preprocess.py` (parser family) | 소스별 CSV → 공통 스키마 | 행 리스트 |
| 정규화 | `ml/agent_roles.py` | 요원·맵·팀명 통일 | 정규화된 행 |
| 품질 검사 | `ml/baseline/preprocess.py` | 불완전 행 제거 | 클린 행 리스트 |
| dedup | `ml/baseline/preprocess.py` | 소스 중복 제거 | matches.csv |
| 분할 | `ml/baseline/preprocess.py` | 경기 단위 그룹 분할 | train/val/test |
| 피처 집계 | `ml/baseline/preprocess.py` | train 기준 통계 집계 | 집계 테이블 |
| 피처 생성 | `ml/baseline/preprocess.py` | 피처 생성 (baseline 178 / advanced 128) | features_base.csv |
| sample_weight | `ml/baseline/preprocess.py` | time_weight × source_weight 계산 | train.csv |
...
품질 검사 + dedup 통과: **66,485 맵 행**.
advanced(adv_kaggle_only) 분할: train **53,427행** / test **13,357행**.
...
python -m ml.baseline.preprocess --input data/raw/kaggle --output /tmp/out --reports /tmp/rep
```
...
| [06_feature_engineering.md](06_feature_engineering.md) | baseline 178 / advanced 128 피처 생성 상세 |
