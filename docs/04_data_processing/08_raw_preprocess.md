...
> **구현 완료** — `src/data/raw_preprocess.py`로 생성.
...
현재 웹 스택(FastAPI `src/api` + Next.js `web`)과 active champion 모델은 이 산출물로 교체되지 않았다. 즉, 이 문서의 대상은 “새 raw-only 전처리 결과”이고, “현재 서비스 예측 모델의 활성 입력 계약”은 아니다.
...
```bash
.venv/bin/python -m py_compile src/data/raw_preprocess.py tests/test_raw_preprocess.py
...
- 웹 스택(`src/api`) input contract 변경
