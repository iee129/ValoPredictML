...
실제 파싱은 `src/data/raw_preprocess.py` 내 parser family로 구현되어 있다 (`ml/parsers/` 디렉토리는 존재하지 않음).

| 파서 함수 | 소스 | 구현 위치 |
|-----------|------|----------|
| `parse_ryanluong` | vct_2021_2023, ryanluong challengers | `src/data/raw_preprocess.py` |
| `parse_qualidea` | qualidea1217 | `src/data/raw_preprocess.py` |
| `parse_ediashtarevin` | ediashtarevin | `src/data/raw_preprocess.py` |
| `parse_piyush2024` | piyush86kumar/valorant-champions-2024 | `src/data/raw_preprocess.py` |
| `parse_vlrgg_raw_detail` | vlrgg_* (예정) | `src/data/raw_preprocess.py` |
...
```python
# src/data/raw_preprocess.py parser family 호출
