...
| **언어(프론트)** | **TypeScript** | **5.x** | **타입 안전 API 계약** ← 08_web과 핵심 차이 |
...
## 2. TypeScript를 쓰는 이유 (08_web은 JS로 확정했었음)

초기 설계안은 "JavaScript, TypeScript 미사용"으로 못 박았다. 본 시연은 **TypeScript를 채택**한다. 근거:
...
- 08_web이 겪은 실패(존재하지 않는 피처명 `"팀 조합 다양성"` 같은 임의 필드)를 타입이 구조적으로 차단한다.
...
생성: `npx create-next-app@latest web --typescript --app --tailwind` (상세 → [../03_frontend_nextjs/01_setup_and_structure.md](../03_frontend_nextjs/01_setup_and_structure.md)).
...
| 프론트→백 URL | `VALO_INTERNAL_API_URL` (Next Route Handler 서버 전용, 기본 `http://127.0.0.1:8000`) |
