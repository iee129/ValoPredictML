...
> `agents_a`/`agents_b`는 `"Jett|Sova|..."` 형태(파이프 구분). `src/domain/valorant.py`의 `normalize_agent`로 정규화 후 집계.

### (b) 도메인 룰 fallback (표본 부족 시)

신규/저표본 요원(Miks·Veto·Waylay 등)은 맵 데이터가 적다. 이때 `docs/09_valorant`의 산문을 룩업 테이블로 인코딩해 사용:
- `agents.md` 각 요원 카드 **"강한 맵"** (예: `Jett → Haven, Split, Ascent, Fracture`)
- `maps.md` 각 맵 카드 **"키 픽"** / **"약한 요원"** (예: `Ascent 약한 요원: Brimstone, Phoenix`)

→ `src/api/data/agent_map_rules.json` (손수 1회 인코딩):
```json
{
