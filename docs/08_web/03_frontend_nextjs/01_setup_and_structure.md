...
생성 결과: Next.js 16 App Router + React 19 + **TypeScript** + Tailwind v4. `web/`는 저장소 안 별도 프론트 루트로 둔다(백엔드 `src/api/`와 분리).

> `.gitignore` 허용목록에 프론트를 커밋하려면 `/web/`, `/web/**`를 추가하되 `web/node_modules/`·`.next/`는 제외(create-next-app의 `.gitignore`가 처리).
...
├── .env.local                 ← VALO_INTERNAL_API_URL=http://127.0.0.1:8000
│
└── src/
    ├── app/
    │   ├── layout.tsx          공통 레이아웃 (Navbar)
    │   ├── globals.css         Tailwind @theme
    │   ├── page.tsx            / 홈 (커스텀 5v5 핵심 시연 — 구 /predict 내용 이동)
    │   ├── replay/
...
핵심: `src/types/api.ts`가 **백엔드 `src/api/schemas.py`와 1:1**이다. 계약이 바뀌면 두 파일을 함께 고친다. → [02_types_and_api_client.md](02_types_and_api_client.md).
...
`strict: true`로 두는 게 08_web 실패(계약 어긋남)를 막는 핵심 장치다.
...
브라우저는 항상 Next Route Handler(`/api`)를 호출한다. FastAPI 주소는 서버 전용 환경변수 `VALO_INTERNAL_API_URL`로만 설정한다.
...
| `/model` | 근거 — 179피처, AUC, 검증, 전역 중요도 | 없음 |
