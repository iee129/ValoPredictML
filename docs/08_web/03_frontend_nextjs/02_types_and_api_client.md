...
이 파일이 프론트의 단일 계약 출처다. 백엔드 `src/api/schemas.py`([../02_backend_fastapi/04_schemas.md](../02_backend_fastapi/04_schemas.md))와 필드명·타입이 정확히 일치해야 한다.
...
> **불변량을 타입으로 고정**: `Role` 유니온은 4종, `n_features":179, `team_a/team_b`는 5슬롯. 08_web처럼 임의 필드(`"팀 조합 다양성"`)를 넣으면 컴파일 에러가 난다.
