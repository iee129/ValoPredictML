...
| GET | `/history` | 예측 기록 목록 (pagination) | 없음 |
| GET | `/history/{history_id}` | 예측 기록 상세 (요청/응답 JSON 포함) | 없음 |
...
```
`load_model()`의 `n_features_in_`이 179가 아니면 `model_loaded=false`로 표기(앱 `load_model`은 불일치 시 `ValueError`를 던지므로 503 처리).
...
```json
{
  ...
  "contract": "advanced",
  "n_features": 179,
  "metrics": { "test_auc": 0.7010, "test_acc": 0.6454, "test_f1": 0.6478 },
...
```
전체 13종: Ascent, Bind, Haven, Split, Icebox, Breeze, Fracture, Pearl, Lotus, Sunset, Abyss, Drift, Corrode. (ko 매핑 출처: `src/inference/predict.py` `MAP_LABELS`)
...
| 파라미터 | 타입 | 기본값 | 설명 |
|---------|------|--------|------|
| `limit` | int | 50 | 1~200 (범위 초과 시 clamp) |
...
### 응답 (`HistoryDetailResponse`)

`HistoryItem` 필드 전체 + 원본 요청/응답 JSON 포함:
...
```json
{
  ...
  "team_a_win_probability": 0.62,
  "team_b_win_probability": 0.38,
  "request_json": { "map": "Ascent", "cutoff_year": 2026, "team_a": [...], "team_b": [...] },
  "response_json": { "predicted_winner": "A", "confidence": 0.24, ... }
}
```

없는 `history_id` → 404. DB 미설정 → 503.
