# 02. 모델 서빙 (src/inference/predict.py 재사용)

## 1. 재사용하는 함수 (재구현 금지)

FastAPI는 아래 `src/inference/predict.py` 공개 함수만 호출한다. 시그니처는 실제 코드 그대로다.
...
- **첫 호출**: `_historical_match_inputs`가 `data/processed/{matches,players}.csv` 전체를 읽어 이전연도 누적 상태를 만든다. `matches.csv`의 행은 BO 시리즈가 아니라 맵 단위 승패 샘플 후보이며, 데이터 규모상 수 초~수십 초가 걸릴 수 있다.
...
`src/inference/predict.py`의 `@dataclass PredictionResult`. 응답 직렬화의 단일 출처다.
...
| `feature_values` | dict\|None | 179피처 전체 값 (디버그/심화 표시용) |
...
08_web은 `"팀 조합 다양성"` 같은 **존재하지 않는 라벨**을 넣었다. 실제 `feature`는 `FEATURE_COLS_ADVANCED`의 컬럼명이다. 예:
...
프론트는 `feature` 원본 키를 받고, 한국어 라벨이 필요하면 `src/inference/predict.py`의 `feature_label()`과 동일한 매핑을 자체 보유하거나, 백엔드가 `label` 필드를 추가로 직렬화한다(권장). → [04_schemas.md](04_schemas.md) §3.

---

## 5. 모델 메타·지표 (`/model`)

- `load_model()[1]` = `models/advanced/meta.json`: `algorithm="RF+XGB+LGBM_soft_voting"`, `n_features=179`, `feature_contract="advanced"`, `feature_names`, `validation.final_verdict` 등.
- `load_reports()`:
  - `metrics` = `reports/advanced/metrics.json` → `test_auc`, `test_acc`, F1 등.
  - `validation` = `.../validation.json` → `final_verdict="신뢰 가능"`, 게이트 결과.
- `global_feature_importance(limit)` → 앙상블 멤버 가중 평균 중요도 상위 N.

값은 재학습마다 변하므로 **하드코딩하지 말고** 항상 `/model`로 읽어 표시한다(현재 시간순 split test AUC 0.7010). `/model`의 EDA 데이터 수는 **맵 단위 승패 샘플** 기준이며, BO3·BO5 시리즈 전체 경기 수가 아니다.
