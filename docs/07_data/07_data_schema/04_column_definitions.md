...
## 3. ~~piyush 파서 (2024/2025)~~ (제거됨)

> ❌ **파이프라인에서 제거됨**: `piyush86kumar` 데이터셋 폴더 및 `ml/parsers/piyush.py` 파서가 삭제됨. 아래 컬럼 정의는 역사적 기록으로만 보존.
...
> ⚠️ **파이프라인에서 제거됨**: 리젝션율 80%, 26행만 통과. 아래 컬럼 정의는 역사적 기록으로 보존.
