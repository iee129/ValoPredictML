# 01. 채택 데이터셋 카탈로그

마지막 업데이트: 2026-06-05

---

## 1. Kaggle 원천 데이터셋 명세

현행 advanced 모델은 아래 Kaggle 원천과 VLR.gg 수집 스냅샷을 통합한다. 이 문서는 Kaggle 원천 카탈로그를 정리하며, 최종 학습·평가 행 수는 `reports/ingest_summary.json`과 `data/processed/advanced/{train,test}.csv`를 기준으로 본다.

| # | 등급 | Kaggle ID | 용량 | 파서 | 소스 가중치 | 행 수 (추정) |
|---|------|-----------|------|------|------------|------------|
| 1 | 핵심 | `ryanluong1/valorant-champion-tour-2021-2023-data` | 1.2GB | ryanluong | 1.0 | ~600K |
| 2 | 핵심 | `ryanluong1/valorant-challengers-league-data` | 1.0GB | ryanluong | **1.8** | ~412K |
| 3 | 핵심 | `qualidea1217/valorant-pro-matches-since-april-2021` | ~35MB | qualidea | 1.0 | ~250K |
| 4 | ~~piyush~~ ❌ 제거됨 | `piyush86kumar/valorant-champions-tour-2024-all-events` | ~15MB | ~~piyush~~ (제거됨) | — | ~30K |
| 5 | ~~piyush~~ ❌ 제거됨 | `piyush86kumar/valorant-vct-2025-all-events` | — | ~~piyush~~ (제거됨) | — | ~15K |
| 6 | 보조 | `ediashtarevin/vct-champions-2023-stats` | — | ediashtarevin | 0.9 | ~6K |
| 7 | ~~보조~~ ❌ 제거됨 | `kierru/vctpacific-2023` | — | kierru | 0.9 (제거됨 — 리젝션율 80%로 제거) | ~5K |

**현행 처리 결과**: processed 맵 단위 승패 후보 91,459개, advanced 학습·평가 샘플 91,458개(train 75,405 / test 16,053). 이 값은 BO 시리즈 수가 아니라 맵 단위 승패 샘플 수다.

---

## 2. 소스 가중치 정책

| 가중치 | 소스 | 이유 |
|--------|------|------|
| **1.8** | ryanluong challengers | 컬럼 수 최다, 공수 분리 스탯 보유 |
| 1.0 | vct_2021_2023, qualidea | 대용량 다년도 소스 |
| 0.9 | ediashtarevin, ~~kierru~~ (제거됨) | 특정 대회·지역 보강, 소규모 (kierru는 리젝션율 80%로 파이프라인에서 제거) |

동일 dedup_key 중 가중치 높은 소스의 행을 보존. 동점이면 컬럼 수가 더 많은 행 보존.

---

## 3. 관련성 종합 평가

| 데이터셋 | 관련성 | 이유 |
|----------|--------|------|
| `vct_2021_2023` | 최고 | 6년치 T1 프로 경기 1.2GB, 핵심 학습 소스 |
| `ryanluong1__challengers` | 최고 | T2 대용량 1.0GB, 공수 점수 분리, 가중치 최고 |
| `qualidea1217__*` | 최고 | 249K행, 공수 분리 스탯 유일 소스, 조인 불필요 |
| `piyush86kumar__2024` | ❌ 제거됨 | 데이터셋 폴더 및 파서 제거됨 |
| `piyush86kumar__2025` | ❌ 제거됨 | 데이터셋 폴더 및 파서 제거됨 |
| `ediashtarevin__*` | 보통 | 2023 Champions 특화, 교차 검증용 |
| `kierru__vctpacific-2023` | ❌ 제거됨 | Pacific 지역 보강 목적이었으나 리젝션율 80%, 26행만 통과 — 파이프라인에서 제거됨 |

---

## 4. 다운로드

```bash
python -m data.dataload
```

`~/.kaggle/kaggle.json` 필요. 다운로드 완료 후 `data/raw/kaggle/` (2.3GB, git 제외).
