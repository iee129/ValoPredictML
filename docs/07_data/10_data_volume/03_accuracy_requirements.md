...
| 목표 | 수치 | 조건 |
|------|------|------|
| 최소 목표 | 80% | 과적합 없음 (train-val 차이 <3%) |
| 권장 목표 | 82% | 교차 검증 평균 기준 |
| 이상적 목표 | 84% | 피처 확장 + 대규모 데이터 시 |
...
# 기준: 15개 피처, XGBoost+LightGBM 앙상블
...
```
[ ] 7,000 경기 + 15 피처 → 현재 (~67-72%)
[ ] 20,000 경기 + 15 피처 → 73-76%
[ ] 50,000 경기 + 15 피처 → 78-80%  ← 최소 목표 근접
