...
> 현행 active 데이터 소스는 Kaggle 5개 데이터셋 + VLR.gg 수집 스냅샷이다.
...
|------|------|------|
| Accuracy | 58~65% (추정) | baseline(랜덤) 0.5667 / advanced(시간순) 0.6454 |
| ROC-AUC | 0.62~0.68 (추정) | baseline(랜덤) 0.5943 / advanced(시간순) 0.7010 |
| F1 | — | baseline(랜덤) 0.6072 / advanced(시간순) 0.6478 |

> baseline은 랜덤 Train 80%/Test 20% 분할, advanced는 시간순(train 2020–2025 / test 2026) 분할 기준 실측치다. advanced 행 단위는 91,458개 맵 단위 승패 샘플이다. 위 초기 추정은 파이프라인 완성 전 낙관적 선행연구 외삽치.
...
─────────────────────────────────────────────────
최종 목표 (미달성)                      80~84% (미달성 — 현재 최고 Acc 0.6454, AUC 0.7010)
```
...
# 아래 수치는 초기 낙관적 추정 — 실측: 179피처(advanced) 시간순 분할 기준 Acc 0.6454·AUC 0.7010
...
> **실측**: RF+XGB+LGBM 소프트보팅(2.0:3.0:0.1) 179피처 시간순 분할 기준 Acc 0.6454·AUC 0.7010 (80%+ 미달성)
...
```
[x] baseline 421피처 + LR+DT 소프트보팅(랜덤 80/20) → Acc 0.5667·AUC 0.5943 달성
[x] advanced 179피처 + RF/XGB/LGBM 소프트보팅(시간순) → Acc 0.6454·AUC 0.7010 달성
...
- 목표 Accuracy: 80~84% (현재 최고: 0.6454)
- 목표 ROC-AUC: 0.82+ (현재 최고: 0.7010)
