...
확정 피처 43개(초기 설계 스펙 — 실제 구현: advanced 125피처 / baseline 178피처) 기준으로 중요도 분석 후 불필요 피처 제거. 과적합 방지 및 모델 성능 최적화.

| 상황 | 피처 수 | 정확도 | 과적합 위험 |
|------|---------|----------|---------|
| 초기 설계 기준선 (참고용) | 15 | ~67-72% (추정) | 낮음 |
| 현재 구현 (baseline) | 178 | Acc 0.6290 (실측) | 낮음 |
| 현재 구현 (advanced) | 125 | Acc 0.6958 (실측) | 낮음 |
| 전체 사용 | 115 | ~75-80% | 높음 |
| **선택 후 (목표, 미달성)** | **30~50** | **80-84% (미달성 — 현재 최고 Acc 0.6958, AUC 0.7570)** | **낮음** |
...
| 현재 구현 advanced 125개 (실측) | **Acc 0.6958 (실측)** |
| + 요원 원-핫 54개 | 73~77% |
| + 맵 원-핫/상호작용 | 75~79% |
| + 조합 품질 피처 | 76~80% |
