...
확정 피처 43개(초기 설계 스펙 — 실제 구현: advanced 179피처 / baseline 421피처) 기준으로 중요도 분석 후 불필요 피처 제거. 과적합 방지 및 모델 성능 최적화.

| 상황 | 피처 수 | 정확도 | 과적합 위험 |
|------|---------|----------|---------|
| 초기 설계 기준선 (참고용) | 15 | ~67-72% (추정) | 낮음 |
| 현재 구현 (baseline) | 421 | Acc 0.5667 (실측) | 낮음 |
| 현재 구현 (advanced) | 179 | Acc 0.6454 (실측) | 낮음 |
| 전체 사용 | 115 | ~75-80% (추정) | 높음 |
| **선택 후 (목표, 미달성)** | **30~50** | **80-84% (미달성 — 현재 최고 Acc 0.6454, AUC 0.7010)** | **낮음** |
...
## 3. SHAP 값 기반 분석 (향후 계획 — 미구현)

> 현재 파이프라인은 SHAP를 사용하지 않는다. 피처 중요도는 `feature_importances_`와 `importance×value` 휴리스틱으로 산출한다(진짜 SHAP 아님). 아래 코드는 향후 도입 시 참고용 스펙이다.
...
| 피처 그룹 추가 순서 | 정확도 추정 |
|----------------|---------|
| 초기 설계 기준선 15개 (참고용) | 67~72% |
| 현재 구현 advanced 179개 (실측) | **Acc 0.6454 (실측)** |
| + 요원 원-핫 54개 | 73~77% |
| + 맵 원-핫/상호작용 | 75~79% |
| + 조합 품질 피처 | 76~80% |
| + 피처 선택 최적화 | **80~84% (미달성 목표)** |
