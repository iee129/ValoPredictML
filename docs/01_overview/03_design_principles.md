...
학습 데이터에 없는 요원이 입력되면 `None`을 반환하여 품질 게이트에서 제외합니다.
...
| `ml/data_pipeline.py` | 전처리 파이프라인 진입점 | 모델 학습 금지 |
| `ml/parsers/*.py` | 소스별 CSV 파싱 | 피처 생성 금지 |
| `ml/train_model.py` | 모델 학습 | API 서빙 금지 |
| `ml/evaluate_model.py` | 성능 평가 | DB 접근 금지 |
| `app/streamlit_app.py` | UI 렌더링 + 예측 호출 | 학습 로직 직접 구현 금지 |
...
한 경기(match_key)의 맵들이 train/val에 분산되면 경기 정보가 누수됩니다.
...
val/test 데이터가 집계에 포함되면 미래 정보 누수 — 절대 금지.
