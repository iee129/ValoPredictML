...
train 데이터를 5조각으로 나누고 각 조각을 순서대로 검증셋으로 사용하여 5번 평가 → 평균. `match_key` 단위로 그룹을 설정하여 같은 경기의 행이 train/val에 동시에 들어가지 않게 한다. test.csv는 K-Fold와 완전히 분리 유지 — 최종 평가에만 한 번 사용.

---

## 모델 성능 (완료)

| 모델 | Optuna CV best AUC | Test AUC |
|------|-------------------|---------|
