# 01. Optuna 설치 및 Study 설정 (향후 계획 — 현행 미적용)

마지막 업데이트: 2026-06-04

> ★ **현행 심화 모델은 Optuna를 사용하지 않는다.** RF/XGB/LightGBM은 코드 고정 하이퍼파라미터로 학습하고, soft voting 가중치(RF 2.0 : XGB 3.0 : LGBM 0.1)만 2025 검증 split 기준 grid search로 선택한다. 이 문서 전체는 **향후 자동 HPO 도입 시의 설계·참고 코드**이며, 현행 산출물(`reports/advanced/metrics.json`, 시간순 Test AUC 0.7010)의 성능 수치와는 무관하다.

## 개요

Optuna는 자동 하이퍼파라미터 최적화(HPO) 프레임워크다. 향후 ValoPredictML에서 RF + XGBoost + LightGBM 앙상블 각 구성원의 하이퍼파라미터를 베이지안 최적화로 탐색할 때 사용할 수 있다. 이 문서는 Optuna 설치부터 전체 최적화 실행까지의 참고 코드를 제공한다.
도입 시 K-Fold (K=5)로 train 내에서 교차 검증하며, test는 최종 평가 1회에만 사용한다.
...
if __name__ == "__main__":
    ...
    # 179개 피처 (advanced 계약) — 전체 컬럼 목록은 docs/preprocessing.md 참조
