...
`advanced` 계약은 125개 피처이며 diff 컬럼을 제외하고 `miks`를 포함한 29개 요원 count를 사용한다.
...
| `models/advanced/meta.json` | 125피처 계약, feature names, split/source 계약, 성능, validation verdict |
| `reports/adv_kaggle_only/metrics.json` | 현재 모델 평가 결과 |
| `reports/adv_kaggle_only/validation.json` | leakage/source/model-contract 검증 결과 |

## 현재 성능

| 지표 | baseline | adv_kaggle_only |
|---|---:|---:|
| Feature count | 178 | 125 |
| Modeling rows | 56,767 | 56,767 |
| Test rows | 10,017 | 10,017 |
| Test ROC-AUC | 0.6562 | **0.7664** (+0.1103) |
| Test Accuracy | 0.6290 | **0.7040** |
| Test F1 | 0.7243 | **0.7698** |
...
- `models/advanced/*.joblib`의 `n_features_in_`는 반드시 125여야 한다.
- `models/advanced/meta.json`의 `feature_names`는 `ml.baseline.preprocess.FEATURE_COLS_ADVANCED` 순서와 같아야 한다.
- `reports/adv_kaggle_only/validation.json`은 split overlap 0, forbidden feature 0, 모든 source `kaggle_` prefix를 통과해야 한다.
