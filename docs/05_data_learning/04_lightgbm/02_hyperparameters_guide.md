...
LightGBM은 RF + XGBoost + LightGBM 가중 soft voting 앙상블 구성원 중 하나다. 스케일링 불필요.

> 현행 심화 LightGBM은 **코드 고정 하이퍼파라미터**를 사용한다(Optuna 미사용): `n_estimators=1000`, `num_leaves=63`, `learning_rate=0.02`, `min_child_samples=40`, `subsample=0.8`, `colsample_bytree=0.7`. 아래 권장 범위·탐색 공간은 일반 가이드이자 향후 자동 HPO 도입 시 참고용이다.
...
ValoPredictML (d=179, advanced 계약):
...
### 9.2 현행 심화 LightGBM 고정 하이퍼파라미터

```python
# 현행 코드 고정값 (Optuna 미사용) — reports/advanced/metrics.json 산출 기준
current_lgbm_params = {
    "num_leaves": 63,           # 복잡한 패턴 허용
    "min_child_samples": 40,    # 리프 최소 샘플 (강한 과적합 방지)
    "learning_rate": 0.02,      # 낮은 학습률 + 많은 트리
    "n_estimators": 1000,
    "subsample": 0.8,
    "colsample_bytree": 0.7,
}
# 이 설정으로 시간순 split LGBM Test AUC ≈ 0.7015
```
