...
LightGBM은 RF + XGBoost + LightGBM 가중 soft voting 앙상블 구성원 중 하나다. 스케일링 불필요. 현재 활성 파이프라인은 `sample_weight` 미사용(균등 학습)이며, 하이퍼파라미터는 코드 고정값을 따른다(Optuna 미사용). 아래 함수의 `sample_weight` 인자는 옵션(None이면 균등 가중)이다.
...
def get_default_lgbm_params() -> dict:
    """
    ValoPredictML 현행 LightGBM 파라미터 (코드 고정값, Optuna 미사용).
    """
    return {
        # 핵심 구조 (가장 중요)
        "num_leaves": 63,
