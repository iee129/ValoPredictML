# 02. Random Forest 구현 (심화 앙상블 구성원)

마지막 업데이트: 2026-06-04

## 개요

Random Forest는 **심화 앙상블(RF + XGBoost + LightGBM) 메인 모델 중 하나**다. 세 모델의 예측 확률을 가중 평균(RF 2.0 : XGB 3.0 : LGBM 0.1)하여 최종 승률을 산출한다. 스케일링 불필요 — 트리 기반 모델은 피처 스케일에 무관하다.

> 주의: 강의 산출물(PDF) 기준 **베이스라인은 LR + DT soft voting**뿐이며 Random Forest는 베이스라인이 아니다. RF는 심화 모델의 구성원으로 위치한다. PDF 베이스라인 성능은 [01_logistic_regression.md](01_logistic_regression.md) 참조.

현행 심화 RF 고정 하이퍼파라미터: `n_estimators=500`, `max_depth=12`, `min_samples_leaf=20` (Optuna 미사용).
...
핵심 파라미터:
- n_estimators (T): 트리 수
- max_features (m): 분기 시 고려할 피처 수 (보통 sqrt(d) = sqrt(179) ≈ 13 (advanced 계약))
...
def train_random_forest(
    ...
    Args:
        X_train: 학습 피처 (N_train, 179)  # advanced 계약
        y_train: 학습 레이블
        X_val: 검증 피처
        y_val: 검증 레이블
...
def plot_learning_curve(rf, X, y):
    ...
    # 데이터 수집 권고 (0.82는 미달성 aspiration 목표 — 현재 심화 앙상블 시간순 Test AUC 0.7010)
    if val_mean[-1] < 0.82:
        gap = val_mean[-2:]
...
심화 시간순 split 실측 결과 (train 2020–2025 = 75,405 / test 2026 = 16,053, 179피처, 맵 단위 승패 샘플):

| 지표 | **Random Forest** | RF+XGB+LGBM 가중 앙상블 |
|------|------------------:|------------------------:|
| Test ROC-AUC | **0.6965** | **0.7010** |
| Test Accuracy | — | **0.6454** |
| Test F1-Score | — | **0.6478** |
| 학습 시간 | **~10초** | ~35초 (3모델 합산) |
| OOB 검증 | **있음 (무료)** | 없음 |

**결론**: Random Forest(Test AUC 0.6965)는 단독으로도 비선형 패턴 포착 능력이 있으며, XGB(0.7007)·LGBM(0.7015)과 가중 결합해 앙상블 AUC 0.7010을 만든다.
앙상블 구성원으로 피처 중요도 검증 1단계(`feature_importances_`)에도 활용.
평가 지표: Accuracy, ROC-AUC, F1 (`reports/advanced/metrics.json`).

## 향후 목표(미달성)

- 목표 AUC: 0.82 이상 (현재 심화 앙상블 시간순 Test AUC 0.7010으로 미달성)
- 목표 Accuracy: 0.80 이상 (현재 0.6454로 미달성)
