...
XGBoost는 RF + XGBoost + LightGBM 가중 soft voting 앙상블 구성원 중 하나다.
트리 기반이라 스케일링 불필요. 현재 활성 파이프라인은 `sample_weight` 미사용(균등 학습)이며, 하이퍼파라미터는 `src/ml/advanced/ensemble.py`의 **코드 고정값**을 따른다(Optuna 미사용). 아래 학습 함수의 early stopping/eval_set 구조는 일반 구현 예시다.
...
def train_xgboost(
    ...
    Args:
        X_train: 학습 피처 DataFrame (N_train, 179)  # advanced 계약
        y_train: 학습 레이블 (0: 패, 1: 승)
        X_val: 검증 피처 DataFrame (N_val, 179)  # advanced 계약
        y_val: 검증 레이블
...
class XGBoostTrainingLogger:
    ...
    def log_final_result(self, summary: dict):
        ...
        # 0.80/0.82는 미달성 aspiration 목표 — 현재 False (시간순 XGB AUC 0.7007 / 앙상블 Acc 0.6454)
...
def save_training_metadata(training_info: dict, summary: dict,
                            output_path: str = "models/advanced/meta.json"):
    """
    학습 메타데이터를 JSON으로 저장.
    재현성과 버전 관리를 위함.
    """
    metadata = {
        **training_info,
        ...
        # 아래 goals는 미달성 aspiration — 실측: 시간순 XGB Test AUC 0.7007 / 앙상블 Acc 0.6454
        "goals": {"accuracy": 0.80, "roc_auc": 0.82},
        "achieved": {"roc_auc": 0.7007},  # XGB 실측 시간순 Test AUC (advanced)
        "goals_achieved": {
            "accuracy": summary["accuracy_mean"] >= 0.80,  # 현재 False (실측 0.6454)
            "roc_auc": summary["roc_auc_mean"] >= 0.82,    # 현재 False (실측 0.7007)
        }
    }
