...
- **피처 수 d = baseline 178 / advanced 125**: 역할군 카운트·파생, 선수 스탯, 시너지, 요원 조합, 맵 피처 전체 (상세 내역은 `preprocessing.md` 7장 참조)
...
- 피처 수 d = 125 (advanced) → 신경망의 복잡한 표현 능력 과잉
...
### 7.1 실제 측정 성능 (advanced, adv_kaggle_only 기준)

| 모델 | Optuna CV best AUC | Test AUC | Test Acc | Test F1 | ValoPredictML 역할 |
|------|-------------------|---------|---------|--------|-------------------|
| Random Forest | 0.6901 | 0.7013 | — | — | **앙상블 메인** |
| XGBoost | 0.7465 | 0.7641 | — | — | **앙상블 메인** |
| LightGBM | 0.7139 | 0.7332 | — | — | **앙상블 메인** |
| **앙상블 (Soft Voting)** | — | **0.7570** | **0.6958** | **0.7649** | **최종 모델** |

데이터: 80/20 (train 53,427 / test 13,357, 125피처, adv_kaggle_only). verdict: `PASS_TRUSTED_KAGGLE_ONLY_ADVANCED`.
...
ValoPredictML의 피처 구조(baseline 178피처 / advanced 125피처, 수치형 위주)와 데이터 규모(VCT 2021~2026, 약 80~100K 맵 행)를 고려할 때:
