...
- **피처 수 d = 15**: Duelist/Initiator/Controller/Sentinel 카운트 × 2팀 + diff × 4 + has_controller × 2 + map_encoded
...
| ValoPredictML 역할 | Baseline (하한선 설정) |
...
| ValoPredictML 역할 | Baseline+ (향상된 기준선) |
...
| ValoPredictML 역할 | 메인 모델 (앙상블의 60%) |
...
| ValoPredictML 역할 | 서브 모델 (앙상블의 40%) |
...
ValoPredictML의 피처 구조(역할군 카운트 15개, 수치형 위주)와 데이터 규모(VCT 2021-2023)를 고려할 때:

1. **XGBoost**: 높은 정확도 + 해석 가능한 피처 중요도 → 메인 모델
2. **LightGBM**: 빠른 학습 + 메모리 효율 → 서브 모델 (앙상블 다양성 확보)
3. **Soft Voting**: 두 모델의 예측 확률 가중 평균 → 단일 모델보다 안정적인 성능
