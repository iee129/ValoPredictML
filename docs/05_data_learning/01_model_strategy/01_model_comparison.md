...
- **피처 수 d = baseline 421 (PDF 기준) / advanced 179**: 역할군 카운트·파생, 선수 스탯, 시너지, 요원 조합, 맵 피처 전체 (상세 내역은 `preprocessing.md` 7장 참조)
...
- 데이터 크기: 현행 advanced 기준 91,458개 맵 단위 승패 샘플 → 딥러닝보다 트리 기반 tabular 모델이 더 해석 가능하고 관리 비용이 낮음
- 피처 수 d = 179 (advanced) → 신경망의 복잡한 표현 능력 과잉
...
### 7.1 실제 측정 성능 (advanced, 시간순 split 기준)

| 모델 | Test AUC | Test Acc | Test F1 | ValoPredictML 역할 |
|------|---------:|---------:|--------:|-------------------|
| Random Forest | 0.6965 | — | — | **앙상블 메인** |
| XGBoost | 0.7007 | — | — | **앙상블 메인** |
| LightGBM | 0.7015 | — | — | **앙상블 메인** |
| **앙상블 (가중 Soft Voting)** | **0.7010** | **0.6454** | **0.6478** | **최종 모델** |

데이터: 시간순 split (train 2020–2025 = 75,405 / test 2026 = 16,053, 179피처, 맵 단위 승패 샘플). 가중치 RF 2.0 : XGB 3.0 : LGBM 0.1, 가중치 선택 val AUC 0.6682. `final_verdict`: `신뢰 가능`.

참고 베이스라인: LR+DT soft voting (랜덤 80/20, PDF 기준) Test AUC 0.5943 / Acc 0.5667 / F1 0.6072. 베이스라인은 랜덤 분할, 심화는 시간순 분할이므로 AUC를 1:1로 직접 비교하지 않는다.
...
ValoPredictML의 피처 구조(baseline 421피처(PDF) / advanced 179피처, 수치형 위주)와 데이터 규모(VCT 2020~2026, 91,458개 맵 단위 승패 샘플)를 고려할 때:

1. **RF + XGBoost + LightGBM 가중 앙상블**: 세 모델의 예측 확률을 RF 2.0 : XGB 3.0 : LGBM 0.1 가중 평균 → 최종 승률 산출
2. **딥러닝(MLP, PyTorch, TensorFlow) 미사용**: tabular 데이터는 트리 기반 모델이 우위
3. **베이스라인 LR + DT soft voting**: 메인 모델 후보 아님 — 성능 하한선 설정 용도만 (랜덤 80/20)
4. **평가 지표**: Accuracy, ROC-AUC, F1 / 심화는 시간순 split으로 일반화 성능 확인
