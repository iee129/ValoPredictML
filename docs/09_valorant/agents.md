...
요원 분류는 `src/domain/agent_roles.py`의 `AGENT_ROLE_MAP` 기준 **29개**:
...
Miks(Controller)·Veto(Sentinel) 모두 `src/domain/agent_roles.py`의 `AGENT_ROLE_MAP`에 등록 완료. baseline 모델링은 Miks 제외 28명(→84열), advanced는 Miks 포함 29명(→58열).
