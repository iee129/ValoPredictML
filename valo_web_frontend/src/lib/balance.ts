...
// docs_web/06_insights/03_balance_warning.md — 프론트 즉시 룰(백엔드 룰과 동일)
export function balanceCheck(roles: Role[]): BalanceWarning[] {
  const c: Record<Role, number> = { duelist: 0, initiator: 0, controller: 0, sentinel: 0 };
  roles.forEach((r) => { if (r in c) c[r]++; });
