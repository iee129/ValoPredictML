...
def _load_all_splits(input_dir: str) -> dict[str, pd.DataFrame]:
    return {split: load_split(split, base=input_dir) for split in ("train", "val", "test")}
...
def _split_overlap_count(splits: dict[str, pd.DataFrame]) -> int:
    ...
    return int(
        len(keys["train"] & keys["val"])
        + len(keys["train"] & keys["test"])
        + len(keys["val"] & keys["test"])
    )
...
def _label_shuffle_auc(y_true: pd.Series, y_prob: np.ndarray) -> dict:
    ...
    status = "PASS" if 0.48 <= auc <= 0.52 else "WARN"
    ...
    meta.update(
        {
            ...
            "leakage_gate_status": validation["leakage_gate_status"],
...
def validate(input_dir: str = "data/processed", models_dir: str = "models/baseline") -> None:
    ...
    leakage_gate_status = {
        "forbidden_features": "PASS" if not forbidden_features else "FAIL",
        "same_match_leakage": same_year_check["status"],
        "same_year_history_exclusion": same_year_check["status"],
        "split_overlap": "PASS" if split_overlap == 0 else "FAIL",
        "label_shuffle_auc": label_shuffle["status"],
        "single_feature_auc": single_feature["status"],
    }
    hard_fail = (
        bool(forbidden_features)
        or same_year_check["status"] == "FAIL"
        or split_overlap != 0
        or single_feature["status"] == "FAIL"
    )
    final_verdict = (
        "PASS_TRUSTED_PREMATCH_BASELINE"
        if not hard_fail
        else "FAIL_UNTRUSTED_PREMATCH_BASELINE"
    )
    ...
    result = {
        ...
        "same_match_leakage_check": same_year_check,
