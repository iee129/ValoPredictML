...
계약(엔드포인트·필드명)은 실제 api/schemas.py 및 프론트 types/api.ts와 동일하게 맞춘다.
도메인 값(요원 29·맵 13·역할)은 domain.agent_roles에서 가져와 실제와 일치시키고, 승률·피처·근거는
입력으로부터 deterministic하게 생성한 가짜 값이다.

⚠️ 시연/검증 전용. 실제 데이터·모델이 준비되면 api.main 으로 교체한다.

실행: uvicorn api.mock_main:app --host 127.0.0.1 --port 8000
"""
...
from domain.agent_roles import AGENT_ROLE_MAP, MAP_ORDER
...
@app.get("/health")
def health():
    return {"status": "ok", "model_loaded": True, "n_features": 179,
...
def model():
    feats = [
        ("diff_prior_kd_mean", 0.063), ("diff_prior_games_mean", 0.028),
        ("diff_prior_kd_x_history_coverage", 0.024), ("diff_max_prior_kd", 0.020),
        ("diff_prior_fkpr_mean", 0.020), ("diff_prior_adr_mean", 0.019),
        ("diff_player_agent_games_mean", 0.018), ("diff_low_sample_player_ratio", 0.015),
        ("diff_player_agent_kd_mean", 0.012), ("diff_prior_apr_mean", 0.012),
        ("diff_agent_map_fit", 0.010), ("diff_team_games", 0.009),
    ]
    return {
        "algorithm": "RF+XGB+LGBM_soft_voting (MOCK)",
        "contract": "advanced", "n_features": 179,
        "metrics": {"test_auc": 0.7222, "test_acc": 0.6551, "test_f1": 0.6692},
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("api.mock_main:app", host="127.0.0.1", port=8000)
