...
계약(엔드포인트·필드명)은 실제 web.backend/schemas.py 및 프론트 types/api.ts와 동일하게 맞춘다.
도메인 값(요원 29·맵 13·역할)은 ml.agent_roles에서 가져와 실제와 일치시키고, 승률·피처·근거는
입력으로부터 deterministic하게 생성한 가짜 값이다.

⚠️ 시연/검증 전용. 실제 데이터·모델이 준비되면 web.backend.main 으로 교체한다.

실행: uvicorn web.backend.mock_main:app --host 127.0.0.1 --port 8000
"""
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("web.backend.mock_main:app", host="127.0.0.1", port=8000)
